<?php
    ...
    $query ="insert into t_sale(userid,title,content,portid,kindid,stuffid,price,updatetime) values ("
        .$userid.",'".$title."','".$_POST['content']."',".$_POST["areaselect"].","
		.$_POST["kindselect"].",".$_POST['stuffselect'].",".$price.",'".date("Y-m-d H:i:s")."')";
	$result =$conn->query($query);
   if($result)
   {
     echo "<script>alert('信息发布成功！');window.location.href='saleList.php'</script>";
   }else{
